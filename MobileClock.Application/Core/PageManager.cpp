...
namespace mobileclock::application::core {
    ...
    bool PageManager::Navigate(std::string_view pageName, mobileclock::presentation::core::NavigationDirection direction) {
    ...
    bool PageManager::Trigger(NavigationTrigger trigger) {
        ...
        interface::IPage* const target = this->pages.Find(route->target);
        if (target == nullptr) {
            LOG_WARNING(
                "MobileClock.Navigation",
