...
namespace mobileclock::ui::control {
    ...
    std::string_view AlarmMelodyList::RuntimeClassName() const {
        return "mobileclock::ui::control::AlarmMelodyList";
    }
#endif
    ...
    // InteractiveListBase
    //
    interface::GestureHandling AlarmMelodyList::ResolveInteractiveGesture(
        ...
        if (direction == interface::GestureDirection::left) {
            return interface::GestureHandling::captured;
        }
        return direction == interface::GestureDirection::right && this->openedItem == &state.target
            ? interface::GestureHandling::captured : interface::GestureHandling::ignored;
    }

    void AlarmMelodyList::BeginInteractiveGesture(const interface::IGestureTarget::PanState& state) {
        if (this->openedItem != nullptr && this->openedItem != &state.target) {
            this->openedItem->SetRenderOffsetX(0.0f);
        }
    }

    void AlarmMelodyList::UpdateInteractiveGesture(const interface::IGestureTarget::PanState& state) {
